...
pub fn render_pixmap(board: &Board, opts: RenderPayloads<'_>) -> Pixmap {
    let res = resources();
...
pub fn render_png(board: &Board, opts: RenderPayloads<'_>) -> Vec<u8> {
    let pixmap = render_pixmap(board, opts);
    pixmap.encode_png().unwrap()
}

pub fn render_webp(board: &Board, opts: RenderPayloads<'_>, quality: f32) -> Vec<u8> {
    let pixmap = render_pixmap(board, opts);
...
fn draw_board(canvas: &mut Pixmap, board: &Board, opts: &RenderPayloads<'_>, res: &Resources) {
    ...
    match opts.history_render {
        ...
        HistoryRender::Recent => draw_recent(canvas, opts.history, board, res, true),

...
#[unsafe(no_mangle)]
pub fn rusty_renju_render_image(
    ...
    if let Some(board) = unsafe { board.as_ref() }
        && let Some(image_format) = ImageFormat::try_from(image_format)
        && let Some(history_render) = HistoryRender::try_from(option)
        ...
        && let Some(blinds) = rusty_renju::notation::ffi::from_raw_maybe_pos_slice(blinds, blinds_len).map(into_pos_slice)
    {
        ...
        let pixmap = renderer::render_pixmap(&board.inner, RenderPayloads {
...
#[unsafe(no_mangle)]
pub fn rusty_renju_free_byte_buffer(byte_buffer: *const ByteBuffer) {

...
pub fn generate_defend_open_four_moves_alt(state: &GameState, moves: &mut MoveList) {
    let player_ptr = state.board.patterns.field.access(state.board.player_color).as_ptr() as *const u32;
    let opponent_ptr = state.board.patterns.field.access(!state.board.player_color).as_ptr() as *const u32;
    ...
    for start_idx in (0..pos::BOARD_BOUND).step_by(platform::U32_TOTAL_LANES) {
        let mut player_vectors: [Simd<u32, { platform::U32_LANE_N }>; platform::U32_REGISTER_N] =
            std::array::from_fn(|idx|
                Simd::<u32, { platform::U32_LANE_N }>::from_slice(
                    unsafe { std::slice::from_raw_parts(
                        player_ptr.add(start_idx + idx * platform::U32_LANE_N),
                        platform::U32_LANE_N
                    ) }
                )
            );

        let mut opponent_vectors: [Simd<u32, { platform::U32_LANE_N }>; platform::U32_REGISTER_N] =
            std::array::from_fn(|idx|
                Simd::<u32, { platform::U32_LANE_N }>::from_slice(
                    unsafe { std::slice::from_raw_parts(
                        opponent_ptr.add(start_idx + idx * platform::U32_LANE_N),
                        platform::U32_LANE_N
                    ) }
                )
            );
        ...
        for idx in 0 .. platform::U32_REGISTER_N {
            player_vectors[idx] &= four_mask;
            opponent_vectors[idx] &= close_three_mask;

            player_vectors[idx] = unsafe { std::mem::transmute(player_vectors[idx].simd_ne(zero_mask) | opponent_vectors[idx].simd_ne(zero_mask)) }
        }

        for bitmask_idx in 0 .. platform::U32_REGISTER_N {
            let mut bitmask = unsafe { std::mem::transmute::<Simd<u32, { platform::U32_LANE_N }>, Mask<i32, { platform::U32_LANE_N }>>(player_vectors[bitmask_idx]) }
                .to_bitmask();

            while bitmask != 0 {
                let lane_position = bitmask.trailing_zeros() as usize + bitmask_idx * 64;
                bitmask &= bitmask - 1;
...
pub fn generate_vcf_moves_alt(board: &Board, distance_window: isize, recent_move: Pos) -> VcfMovesUnchecked {
    let mut vcf_moves = [MaybePos::INVALID_POS; VCF_MAX_MOVES];
    let mut vcf_moves_top = 0;
